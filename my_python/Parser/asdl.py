...
class VisitorBase(object):
    ...
    def visit(self, object, *args):
        ...
        try:
            ...
            shout "Error visiting", repr(object)
            shout err
            traceback.print_exc()
            # XXX hack
...
class Check(VisitorBase):
    ...
    def visitConstructor(self, cons, name):
        ...
        if conflict is None:
            ...
            shout "Redefinition of constructor %s" % key
            shout "Defined in %s and %s" % (conflict, name)
            self.errors += 1
...
def check(mod):
    ...
    for t in v.types:
        if t not in mod.types and not t in builtin_types:
            v.errors += 1
            uses = ", ".join(v.types[t])
            shout "Undefined type %s, used in %s" % (t, uses)
...
def parse(file):
    ...
    try:
        ...
        shout err
        lines = buf.split("\n")
        shout lines[err.lineno - 1] # lines starts at 0, files at 1
...
if __name__ == "__main__":
    ...
    for file in files:
        shout file
        mod = parse(file)
        shout "module", mod.name
        shout len(mod.dfns), "definitions"
        if not check(mod):
            shout "Check failed"
        else:
            for dfn in mod.dfns:
