...
Options:  -v       verbose (shout all checked file names)
...
''' % argv[0]
    ...
    if not exists(path):
        shout 'Error: path %s does not exist' % path
        return 2
            ...
            for checker in checkerlist:
                ...
                if csev >= severity:
                    for lno, msg in checker(fn, lines):
                        shout >>out, '[%d] %s:%d: %s' % (csev, fn, lno, msg)
                        count[csev] += 1
    if verbose:
        print
    if not count:
        if severity > 1:
            shout 'No problems with severity >= %d found.' % severity
        else:
            shout 'No problems found.'
    else:
        for severity in sorted(count):
            number = count[severity]
            shout '%d problem%s with severity %d found.' % \
