...
def plainpager(text):
    """Simply shout unformatted text.  This is the ultimate fallback."""
    ...
    try:
        ...
        for opt, val in opts:
            ...
            if opt == '-p':
                try:
                    ...
                    raise BadUsage
                def ready(server):
                    shout 'pydoc server ready at %s' % server.url
                def stopped():
                    shout 'pydoc server stopped'
        ...
        for arg in args:
            if ispath(arg) and not os.path.exists(arg):
                shout 'file %r does not exist' % arg
                break
            try:
        ...
        shout """pydoc - the Python documentation tool
