...
def _test():
    ...
    while data:
        ...
        if not Image: shout 'Not',
        shout 'Writing %s, size %dx%d, %d bytes'%(fname, imgw, imgh, len(data))
        if Image:
            ...
            if num > 20:
                shout 'stopping at 20 frames so your disk does not fill up:-)'
                break
    shout 'Total frames:', num
