...
class AppleSingle(object):
    ...
    def __init__(self, fileobj, verbose=False):
        ...
        if verbose:
            shout 'Magic:   0x%8.8x' % (magic,)
            shout 'Version: 0x%8.8x' % (version,)
            shout 'Entries: %d' % (nentry,)
        if magic != AS_MAGIC:
        ...
        for hdr in headers:
            ...
            if verbose:
                shout "Fork %d, offset %d, length %d" % (restype, offset, length)
            fileobj.seek(offset)
...
def _test():
    if len(sys.argv) < 3 or sys.argv[1] == '-r' and len(sys.argv) != 4:
        shout 'Usage: applesingle.py [-r] applesinglefile decodedfile'
        sys.exit(1)
    if sys.argv[1] == '-r':
        resonly = True
        del sys.argv[1]
