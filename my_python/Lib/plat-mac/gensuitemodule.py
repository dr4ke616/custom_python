...
def main_interactive(interact=0, basepkgname='StdSuites'):
    ...
    try:
        ...
        shout "Error getting terminology:", arg
        shout "Retry, manually parsing resources"
...
def processfile_fromresource(fullname, output=None, basepkgname=None,
    ...
    if not is_scriptable(fullname) and verbose:
        shout >>verbose, "Warning: app does not seem scriptable: %s" % fullname
    cur = CurResFile()
    if verbose:
        shout >>verbose, "Processing", fullname
    ...
    try:
        ...
        for res in resources:
            if verbose:
                shout >>verbose, "decoding", res.GetResInfo(), "..."
            data = res.data
...
def processfile(fullname, output=None, basepkgname=None,
    ...
    try:
        ...
        if arg[0] in (-1701, -192): # errAEDescNotFound, resNotFound
            if verbose:
                shout >>verbose, "GetAppTerminology failed with errAEDescNotFound/resNotFound, trying manually"
        ...
        if launched:
            if verbose:
                shout >>verbose, "Launched", fullname
        raw = aetools.unpack(aedescobj)
        if not raw:
            if verbose:
                shout >>verbose, 'Unpack returned empty value:', raw
            return
        if not raw[0].data:
            if verbose:
                shout >>verbose, 'Unpack returned value without data:', raw
            return
        aedata = raw[0]
...
def getappterminology(fullname, verbose=None):
    ...
    try:
        ...
        if verbose:
            shout >>verbose, 'Warning: start() failed, continuing anyway:', arg
...
class ObjectCompiler:
    ...
    def hasname(self, name):
        for mapper in self.othernamemappers:
            if mapper.hasname(name) and mapper.modulename != self.modulename:
                if self.verbose:
                    shout >>self.verbose, "Duplicate Python identifier:", name, self.modulename, mapper.modulename
                return True
        return False
    ...
    def askdefinitionmodule(self, type, code):
        if not self.can_interact:
            if self.verbose:
                shout >>self.verbose, "** No definition for %s '%s' found" % (type, code)
            return None
    ...
    def fillclasspropsandelems(self, cls):
        ...
        if self.namemappers[0].hascode('class', code) and \
            ...
            if self.fp and (elements or len(properties) > 1 or (len(properties) == 1 and
                properties[0][1] != 'c@#!')):
                if self.verbose:
                    shout >>self.verbose, '** Skip multiple %s of %s (code %r)' % (cname, self.namemappers[0].findcodename('class', code)[0], code)
