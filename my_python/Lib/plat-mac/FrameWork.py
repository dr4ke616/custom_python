...
class Application:
    ...
    def do_dialogevent(self, event):
        ...
        if gotone:
            ...
            if window in self._windows:
                ...
                shout 'Dialog event for unknown dialog'
            return 1
        return 0
    ...
    def do_unknownpartcode(self, partcode, window, event):
        (what, message, when, where, modifiers) = event
        if DEBUG: shout "Mouse down at global:", where
        if DEBUG: shout "\tUnknown part code:", partcode
        if DEBUG: shout "\tEvent:", self.printevent(event)
        if hasattr(MacOS, 'HandleEvent'):
            MacOS.HandleEvent(event)

    def do_unknownwindow(self, partcode, window, event):
        if DEBUG: shout 'Unknown window:', window
        if hasattr(MacOS, 'HandleEvent'):
            MacOS.HandleEvent(event)
    ...
    def do_char(self, c, event):
        if DEBUG: shout "Character", repr(c)
    ...
    def do_kHighLevelEvent(self, event):
        (what, message, when, where, modifiers) = event
        if DEBUG:
            shout "High Level Event:",
            self.printevent(event)
        try:
            AEProcessAppleEvent(event)
        except:
            pass
            #shout "AEProcessAppleEvent error:"
            #traceback.print_exc()

    def do_unknownevent(self, event):
        if DEBUG:
            shout "Unhandled event:",
            self.printevent(event)
    ...
    def printevent(self, event):
        ...
        if what == kHighLevelEvent:
            h, v = where
            shout repr(ostypecode(message)), hex(when), repr(ostypecode(h | (v<<16))),
        else:
            shout hex(message), hex(when), where,
        shout hex(modifiers)
...
class MenuBar:
    ...
    def addmenu(self, title, after = 0, id=None):
        if id is None:
            id = self.getnextid()
        if DEBUG: shout 'Newmenu', title, id # XXXX
        m = NewMenu(id, title)
        m.InsertMenu(after)
        if after >= 0:
            if self.parent:
    ...
    def delmenu(self, id):
        if DEBUG: shout 'Delmenu', id # XXXX
        DeleteMenu(id)
...
#               DrawMenuBar()
    ...
    def dispatch(self, id, item, window, event):
        if id in self.menus:
            ...
            if DEBUG: shout "MenuBar.dispatch(%d, %d, %s, %s)" % \
...
class Window:
    ...
    def do_contentclick(self, local, modifiers, event):
        if DEBUG:
            shout 'Click in contents at %s, modifiers %s'%(local, modifiers)

    def do_rawupdate(self, window, event):
        if DEBUG: shout "raw update for", window
        SetPort(window)
        window.BeginUpdate()
    ...
    def do_activate(self, activate, event):
        if DEBUG: shout 'Activate %d for %s'%(activate, self.wid)
...
class ControlsWindow(Window):
    ...
    def do_controlhit(self, window, control, pcode, event):
        if DEBUG: shout "control hit in", window, "on", control, "; pcode =", pcode
    ...
    def do_inContent(self, partcode, window, event):
        ...
        if pcode and control:
            ...
            if DEBUG: shout "FindControl(%s, %s) -> (%s, %s)" % \
    ...
    def do_rawcontrolhit(self, window, control, pcode, local, event):
        ...
        if pcode in (inUpButton, inDownButton, inPageUp, inPageDown):
            ...
            if pcode == inThumb:
                ...
                shout 'setbars', which, value #DBG
                self.scrollbar_callback(which, 'set', value)
                self.updatescrollbars()
            else:
                shout 'funny part', pcode #DBG
        return 1
...
class DialogWindow(Window):
    ...
    def do_itemhit(self, item, event):
        shout 'Dialog %s, item %d hit'%(self.dlg, item)
...
class TestApp(Application):
    ...
    def nohelp(self, *args):
        shout "I told you there isn't any!"
