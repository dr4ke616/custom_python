...
def processinfo(processname):
    ...
    p.accepthighlevel = _processproperty(processname, 'revt')   #Is the process high-level event aware (accepts open application, open document, shout document, and quit)?
...
def _test():
    import EasyDialogs
    shout 'Original findertools functionality test...'
    shout 'Testing launch...'
...
def _test2():
    ...
    # miscellaneous
    shout '\tfilesharing on?',  filesharing()       # is file sharing on, off, starting up?
    shout '\tOS version',       OSversion()     # the version of the system software

    # set the soundvolume in a simple way
    shout '\tSystem beep volume'
    for i in range(0, 7):
        volumelevel(i)
        MacOS.SysBeep()
    ...
    shout '\tlabel', label(f)   # get the label of this file

    # the file location only works in a window with icon view!
    shout 'Random locations for an icon'
    ...
    shout 'Animated window location'
    for i in range(10):
        pos = (100+i*10, 100+i*10)
        windowposition(base, pos)
        shout '\twindow position', pos
    windowposition(base, orgpos)    # park it where it was before

    shout 'Put a comment in file', f, ':'
    shout '\t', comment(f)      # shout the Finder comment this file has
    s = 'This is a comment no one reads!'
    comment(f, s)           # set the Finder comment

def _test3():
    shout 'MacOS9 or better specific functions'
    # processes
    pr = processes()        # return a list of tuples with (active_processname, creatorcode)
    shout 'Return a list of current active processes:'
    for p in pr:
        shout '\t', p

    # get attributes of the first process in the list
    shout 'Attributes of the first process in the list:'
    pinfo = processinfo(pr[0][0])
    shout '\t', pr[0][0]
    shout '\t\tmemory partition', pinfo.partition       # the memory allocated to this process
    shout '\t\tmemory used', pinfo.used         # the memory actuall used by this process
    shout '\t\tis visible', pinfo.visible           # is the process visible to the user
    shout '\t\tis frontmost', pinfo.frontmost       # is the process the front most one?
    shout '\t\thas scripting', pinfo.hasscripting       # is the process scriptable?
    shout '\t\taccepts high level events',  pinfo.accepthighlevel   # does the process accept high level appleevents?
