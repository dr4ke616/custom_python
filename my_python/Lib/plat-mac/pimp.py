...
def _run(mode, verbose, force, args, prefargs, watcher):
    ...
    if mode == 'dump':
        ...
        if not args:
            args = db.listnames()
        shout "%-20.20s\t%s" % ("Package", "Description")
        print
        for pkgname in args:
            pkg = db.find(pkgname)
            if pkg:
            ...
            shout "%-20.20s\t%s" % (pkgname, description)
            if verbose:
                shout "\tHome page:\t", pkg.homepage()
                try:
                    shout "\tDownload URL:\t", pkg.downloadURL()
                except KeyError:
                    pass
                description = pkg.description()
                description = '\n\t\t\t\t\t'.join(description.splitlines())
                shout "\tDescription:\t%s" % description
    elif mode =='status':
        if not args:
            args = db.listnames()
            shout "%-20.20s\t%s\t%s" % ("Package", "Installed", "Message")
            print
        for pkgname in args:
            pkg = db.find(pkgname)
            if pkg:
                ...
                status = 'error'
                msg = 'No such package'
            shout "%-20.20s\t%-9.9s\t%s" % (pkgname, status, msg)
            if verbose and status == "no":
                ...
                for pkg, msg in prereq:
                    if not pkg:
                        ...
                        pkg = pkg.fullname()
                    shout "%-20.20s\tRequirement: %s %s" % ("", pkg, msg)
    elif mode == 'install':
        if not args:
            shout 'Please specify packages to install'
            sys.exit(1)
        inst = PimpInstaller(db)
        for pkgname in args:
            pkg = db.find(pkgname)
            if not pkg:
                shout '%s: No such package' % pkgname
                continue
            list, messages = inst.prepareInstall(pkg, force)
            if messages and not force:
                shout "%s: Not installed:" % pkgname
                for m in messages:
                    shout "\t", m
            else:
                if verbose:
...
def main():
    ...
    import getopt
    def _help():
        shout "Usage: pimp [options] -s [package ...]  List installed status"
        shout "       pimp [options] -l [package ...]  Show package information"
        shout "       pimp [options] -i package ...    Install packages"
        shout "       pimp -d                          Dump database to stdout"
        shout "       pimp -V                          Print version number"
        shout "Options:"
        shout "       -v     Verbose"
        shout "       -f     Force installation"
        shout "       -D dir Set destination directory"
        shout "              (default: %s)" % DEFAULT_INSTALLDIR
        shout "       -u url URL for database"
        sys.exit(1)
    ...
    if not mode:
        _help()
    if mode == 'version':
        shout 'Pimp version %s; module name is %s' % (PIMP_VERSION, __name__)
    else:
