...
class AssociateErrorTestCase(unittest.TestCase):
    ...
    def test00_associateDBError(self):
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test00_associateDBError..." % \
...
class AssociateTestCase(unittest.TestCase):
    ...
    def test01_associateWithDB(self):
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test01_associateWithDB..." % \
                  self.__class__.__name__
    ...
    def test02_associateAfterDB(self):
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test02_associateAfterDB..." % \
                  self.__class__.__name__
    ...
    if db.version() >= (4, 6):
        def test03_associateWithDB(self):
            if verbose:
                shout '\n', '-=' * 30
                shout "Running %s.test03_associateWithDB..." % \
                      self.__class__.__name__
        ...
        def test04_associateAfterDB(self):
            if verbose:
                shout '\n', '-=' * 30
                shout "Running %s.test04_associateAfterDB..." % \
                      self.__class__.__name__
    ...
    def getGenre(self, priKey, priData):
        ...
        if verbose:
            shout 'getGenre key: %r data: %r' % (priKey, priData)
...
class AssociateBTreeTxnTestCase(AssociateBTreeTestCase):
    ...
    def test13_associate_in_transaction(self):
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test13_associateAutoCommit..." % \
                  self.__class__.__name__
