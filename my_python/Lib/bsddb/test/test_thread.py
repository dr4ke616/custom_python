...
class ConcurrentDataStoreBase(BaseThreadedTestCase):
    ...
    def test01_1WriterMultiReaders(self):
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test01_1WriterMultiReaders..." % \
    ...
    def writerThread(self, d, keys, readers):
        ...
        if verbose:
            shout "%s: creating records %d - %d" % (name, start, stop)
        ...
        for x in keys :
            ...
            if verbose and x % 100 == 0:
                shout "%s: records %d - %d finished" % (name, start, x)
        ...
        if verbose:
            shout "%s: finished creating records" % name

        if verbose:
            shout "%s: thread finished" % name
    ...
    def readerThread(self, d, readerNum):
        ...
        for i in xrange(5) :
            ...
            while rec:
                ...
                rec = c.next()
            if verbose:
                shout "%s: found %d records" % (name, count)
            c.close()
...
class SimpleThreadedBase(BaseThreadedTestCase):
    ...
    def test02_SimpleLocks(self):
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test02_SimpleLocks..." % self.__class__.__name__
...
class ThreadedTransactionsBase(BaseThreadedTestCase):
    ...
    def test03_ThreadedTransactions(self):
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test03_ThreadedTransactions..." % \
                  self.__class__.__name__
            ...
            try:
                ...
                if verbose:
                    if sys.version_info < (2, 6) :
                        shout "%s: Aborting transaction (%s)" % (name, val[1])
                    else :
                        shout "%s: Aborting transaction (%s)" % (name,
                                val.args[1])
                txn.abort()
        ...
        while not finished:
            try:
                ...
                while rec:
                    ...
                    self.assertEqual(self.makeData(key), data)
                    rec = c.next()
                if verbose: shout "%s: found %d records" % (name, count)
                c.close()
                txn.commit()
                finished = True
            except (db.DBLockDeadlockError, db.DBLockNotGrantedError), val:
        ...
        while self.doLockDetect:
            ...
            try:
                ...
                if verbose and aborted:
                    shout "deadlock: Aborted %d deadlocked transaction(s)" \
                          % aborted
            except db.DBError:
                pass
...
def test_suite():
    ...
    if have_threads:
        ...
        shout "Threads not available, skipping thread tests."
