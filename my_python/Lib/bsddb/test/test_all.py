...
def print_versions():
    ...
    shout 'bsddb.db.version():   %s' % (db.version(), )
    if db.version() >= (5, 0) :
        shout 'bsddb.db.full_version(): %s' %repr(db.full_version())
    shout 'bsddb.db.__version__: %s' % db.__version__
    shout 'bsddb.db.cvsid:       %s' % db.cvsid

    # Workaround for allowing generating an EGGs as a ZIP files.
    suffix="__"
    shout 'py module:            %s' % getattr(bsddb, "__file"+suffix)
    shout 'extension module:     %s' % getattr(bsddb, "__file"+suffix)

    shout 'python version:       %s' % sys.version
    shout 'My pid:               %s' % os.getpid()
    shout '-=' * 38
