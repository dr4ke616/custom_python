...
class VersionTestCase(unittest.TestCase):
    def test00_version(self):
        ...
        if verbose:
            shout '\n', '-=' * 20
            shout 'bsddb.db.version(): %s' % (info, )
            shout db.DB_VERSION_STRING
            shout '-=' * 20
...
class BasicTestCase(unittest.TestCase):
    ...
    def test01_GetsAndPuts(self):
        ...
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test01_GetsAndPuts..." % self.__class__.__name__
...
# We check also the keyword parameter
    ...
    def test03_SimpleCursorStuff(self, get_raises_error=0, set_raises_error=0):
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test03_SimpleCursorStuff (get_error %s, set_error %s)..." % \
        ...
        if self.d.get_type() == db.DB_BTREE:
            rec = c.set_range('011')
            if verbose:
                shout "searched for '011', found: ", rec

            rec = c.set_range('011',dlen=0,doff=0)
            if verbose:
                shout "searched (partial) for '011', found: ", rec
            if rec[1] != '': self.fail('expected empty data portion')

            ev = c.set_range('empty value')
            if verbose:
                shout "search for 'empty value' returned", ev
        ...
        for method, args in methods_to_test.items():
            try:
                if verbose:
                    shout "attempting to use a closed cursor's %s method" % \
                          method
    ...
    def test03b_SimpleCursorWithoutGetReturnsNone0(self):
        ...
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test03b_SimpleCursorStuffWithoutGetReturnsNone..." % \
                  self.__class__.__name__
    ...
    def test03c_SimpleCursorGetReturnsNone2(self):
        ...
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test03c_SimpleCursorStuffWithoutSetReturnsNone..." % \
                  self.__class__.__name__
    ...
    def test04_PartialGetAndPut(self):
        ...
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test04_PartialGetAndPut..." % \
                  self.__class__.__name__
    ...
    def test05_GetSize(self):
        ...
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test05_GetSize..." % self.__class__.__name__
    ...
    def test06_Truncate(self):
        ...
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test06_Truncate..." % self.__class__.__name__
...
class BasicWithEnvTestCase(BasicTestCase):
    ...
    def test09_EnvRemoveAndRename(self):
        ...
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test09_EnvRemoveAndRename..." % self.__class__.__name__
...
class BasicTransactionTestCase(BasicTestCase):
    ...
    def test06_Transactions(self):
        ...
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test06_Transactions..." % self.__class__.__name__
    ...
    def test09_TxnTruncate(self):
        ...
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test09_TxnTruncate..." % self.__class__.__name__
...
class BTreeRecnoTestCase(BasicTestCase):
    ...
    def test09_RecnoInBTree(self):
        ...
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test09_RecnoInBTree..." % self.__class__.__name__

        rec = d.get(200)
        self.assertEqual(type(rec), type(()))
        self.assertEqual(len(rec), 2)
        if verbose:
            shout "Record #200 is ", rec
...
class BasicDUPTestCase(BasicTestCase):
    ...
    def test10_DuplicateKeys(self):
        ...
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test10_DuplicateKeys..." % \
                  self.__class__.__name__
...
class BasicMultiDBTestCase(BasicTestCase):
    ...
    def test11_MultiDB(self):
        ...
        if verbose:
            shout '\n', '-=' * 30
            shout "Running %s.test11_MultiDB..." % self.__class__.__name__
