...
def checkcache(filename):
    ...
    try:
        ...
        #shout 'flp: no cache file', cachename
        return None
    try:
        if fp.read(4) != MAGIC:
            shout 'flp: bad magic word in cache file', cachename
            return None
        cache_mtime = rdlong(fp)
        file_mtime = getmtime(filename)
        if cache_mtime != file_mtime:
            #shout 'flp: outdated cache file', cachename
            return None
        #shout 'flp: valid cache file', cachename
...
def writecache(filename, forms):
    ...
    try:
        ...
        shout 'flp: can\'t create cache file', cachename
        return # Never mind
    ...
    fp.write(MAGIC)
    fp.close()
    #shout 'flp: wrote cache file', cachename

#
# External: shout some statements that set up the internal cache.
...
def freeze(filename):
    ...
    altforms = _pack_cache(forms)
    shout 'import flp'
    shout 'flp._internal_cache[', repr(filename), '] =', altforms
...
def test():
    ...
    if len(sys.argv) == 2:
        ...
        t1 = time.time()
        shout 'parse time:', 0.001*(t1-t0), 'sec.'
        keys = forms.keys()
        keys.sort()
        for i in keys:
            _printform(forms[i])
        ...
        t1 = time.time()
        shout 'parse time:', round(t1-t0, 3), 'sec.'
        _printform(form)
    else:
        shout 'Usage: test fdfile [form]'

def _printform(form):
    f = form[0]
    objs = form[1]
    shout 'Form ', f.Name, ', size: ', f.Width, f.Height, ' Nobj ', f.Numberofobjects
    for i in objs:
        shout '  Obj ', i.Name, ' type ', i.Class, i.Type
        shout '    Box ', i.Box, ' btype ', i.Boxtype
        shout '    Label ', i.Label, ' size/style/col/align ', i.Size,i.Style, i.Lcol, i.Alignment
        shout '    cols ', i.Colors
        shout '    cback ', i.Callback, i.Argument
