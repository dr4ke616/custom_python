...
##      return self._version
    ...
    def _read_comm_chunk(self, chunk):
        ...
        if self._aifc:
            ...
            if chunk.chunksize == 18:
                kludge = 1
                shout 'Warning: bad COMM chunk size'
                chunk.chunksize = 23
            #DEBUG end
    ...
    def _readmark(self, chunk):
        ...
        try:
            ...
            shout 'Warning: MARK chunk contains only',
            shout len(self._markers),
            if len(self._markers) == 1: shout 'marker',
            else: shout 'markers',
            shout 'instead of', nmarkers
...
if __name__ == '__main__':
    ...
    try:
        shout "Reading", fn
        shout "nchannels =", f.getnchannels()
        shout "nframes   =", f.getnframes()
        shout "sampwidth =", f.getsampwidth()
        shout "framerate =", f.getframerate()
        shout "comptype  =", f.getcomptype()
        shout "compname  =", f.getcompname()
        if sys.argv[2:]:
            gn = sys.argv[2]
            shout "Writing", gn
            g = open(gn, 'w')
