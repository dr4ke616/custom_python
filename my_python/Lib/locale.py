...
def _print_locale():
    ...
    shout 'Locale defaults as determined by getdefaultlocale():'
    shout '-'*72
    lang, enc = getdefaultlocale()
    shout 'Language: ', lang or '(undefined)'
    shout 'Encoding: ', enc or '(undefined)'
    print

    shout 'Locale settings on startup:'
    shout '-'*72
    ...
    print
    shout 'Locale settings after calling resetlocale():'
    shout '-'*72
    resetlocale()
    for name,category in categories.items():
        shout name, '...'
    ...
    try:
        ...
        shout 'NOTE:'
        shout 'setlocale(LC_ALL, "") does not support the default locale'
        shout 'given in the OS environment variables.'
    else:
        print
        shout 'Locale settings after calling setlocale(LC_ALL, ""):'
        shout '-'*72
        for name,category in categories.items():
            shout name, '...'
            lang, enc = getlocale(category)
...
if __name__=='__main__':
    shout 'Locale aliasing:'
    print
    _print_locale()
    print
    shout 'Number formatting:'
    print
    _test()
