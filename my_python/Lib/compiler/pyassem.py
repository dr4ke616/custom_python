...
class FlowGraph:
    ...
    def startBlock(self, block):
        if self._debug:
            if self.current:
                shout "end", repr(self.current)
                shout "    next", self.current.next
                shout "    prev", self.current.prev
                shout "   ", self.current.get_children()
            shout repr(block)
        self.current = block
