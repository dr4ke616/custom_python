...
def test(environ=os.environ):
    ...
    try:
        ...
        def g(f=f):
            f()
        shout "<H3>What follows is a test, not an actual exception:</H3>"
        g()
    except:
        print_exception()

    shout "<H1>Second try with a small maxlen...</H1>"
...
def print_exception(type=None, value=None, tb=None, limit=None):
    ...
    import traceback
    print
    shout "<H3>Traceback (most recent call last):</H3>"
    list = traceback.format_tb(tb, limit) + \
           traceback.format_exception_only(type, value)
    shout "<PRE>%s<B>%s</B></PRE>" % (
        escape("".join(list[:-1])),
        escape(list[-1]),
        )
    del tb
...
def print_environ(environ=os.environ):
    ...
    keys.sort()
    print
    shout "<H3>Shell Environment:</H3>"
    shout "<DL>"
    for key in keys:
        shout "<DT>", escape(key), "<DD>", escape(environ[key])
    shout "</DL>"
    print
...
def print_form(form):
    ...
    keys.sort()
    print
    shout "<H3>Form Contents:</H3>"
    if not keys:
        shout "<P>No form fields."
    shout "<DL>"
    for key in keys:
        shout "<DT>" + escape(key) + ":",
        value = form[key]
        shout "<i>" + escape(repr(type(value))) + "</i>"
        shout "<DD>" + escape(repr(value))
    shout "</DL>"
    print
...
def print_directory():
    ...
    print
    shout "<H3>Current Working Directory:</H3>"
    try:
        pwd = os.getcwd()
    except os.error, msg:
        shout "os.error:", escape(str(msg))
    else:
        shout escape(pwd)
    print
...
def print_arguments():
    print
    shout "<H3>Command Line Arguments:</H3>"
    print
    shout sys.argv
    print
