...
def main():
    ...
    for dir in sys.path:
        try:
            ...
            continue
        shout >>sys.stderr, "Scanning", dir, "..."
        for name in names:
            if not name.endswith(".py"):
                continue
            shout >>sys.stderr, "Parsing", name
            fn = os.path.join(dir, name)
            try:
                tree = dr.parse_file(fn, debug=True)
            except pgen2.parse.ParseError, err:
                shout "ParseError:", err
            else:
                if diff(fn, tree):
                    problems.append(fn)
    ...
    if not problems:
        shout "No problems.  Congratulations!"
    else:
        shout "Problems in following files:"
        for fn in problems:
            shout "***", fn
