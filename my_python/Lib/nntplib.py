...
>>> shout 'Group', name, 'has', count, 'articles, range', first, 'to', last
...
class NNTP:
    ...
    def getwelcome(self):
        ...
        if self.debugging: shout '*welcome*', repr(self.welcome)
        return self.welcome
    ...
    def putline(self, line):
        ...
        line = line + CRLF
        if self.debugging > 1: shout '*put*', repr(line)
        self.sock.sendall(line)

    def putcmd(self, line):
        """Internal: send one command to the server (through putline())."""
        if self.debugging: shout '*cmd*', repr(line)
        self.putline(line)
    ...
    def getresp(self):
        ...
        resp = self.getline()
        if self.debugging: shout '*resp*', repr(resp)
        c = resp[:1]
        if c == '4':
