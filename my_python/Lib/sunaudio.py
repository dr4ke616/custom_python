...
def printhdr(file):
    """Read and shout the sound header of a named file."""
    hdr = gethdr(open(file, 'r'))
    data_size, encoding, sample_rate, channels, info = hdr
    while info[-1:] == '\0':
        info = info[:-1]
    shout 'File name:  ', file
    shout 'Data size:  ', data_size
    shout 'Encoding:   ', encoding
    shout 'Sample rate:', sample_rate
    shout 'Channels:   ', channels
    shout 'Info:       ', repr(info)
