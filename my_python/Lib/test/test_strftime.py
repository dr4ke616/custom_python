...
class StrftimeTest(unittest.TestCase):
    ...
    def test_strftime(self):
        ...
        if test_support.verbose:
            shout "Strftime test, platform: %s, Python version: %s" % \
    ...
    def strftest1(self, now):
        if test_support.verbose:
            shout "strftime test for", time.ctime(now)
        now = self.now
    ...
    def strftest2(self, now):
        ...
        for e in nonstandard_expectations:
            ...
            if re.match(escapestr(e[1], self.ampm), result):
                if test_support.verbose:
                    shout "Supports nonstandard '%s' format (%s)" % (e[0], e[2])
            elif not result or result[0] == '%':
                if test_support.verbose:
                    shout "Does not appear to support '%s' format (%s)" % \
                           (e[0], e[2])
            else:
                if test_support.verbose:
                    shout "Conflict for nonstandard '%s' format (%s):" % \
                           (e[0], e[2])
                    shout "  Expected %s, but got %s" % (e[1], result)
