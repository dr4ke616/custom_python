...
class SimpleIMAPHandler(SocketServer.StreamRequestHandler):
    ...
    def _send(self, message):
        if verbose: shout "SENT:", message.strip()
    ...
    def handle(self):
        ...
        while 1:
            ...
            if verbose: shout 'GOT:', line.strip()
...
class BaseThreadedNetworkedTests(unittest.TestCase):
    ...
    def make_server(self, addr, hdlr):
        ...
        if verbose: shout "creating server"
        ...
        if verbose:
            ...
            shout "ADDR =", addr
            shout "CLASS =", self.server_class
            shout "HDLR =", server.RequestHandlerClass
        ...
        t.start()
        if verbose: shout "server running"
        return server, t

    def reap_server(self, server, thread):
        if verbose: shout "waiting for server"
        server.shutdown()
        thread.join()
        if verbose: shout "done"
