...
-h/--help       -- shout this text and exit
...
-S/--slow       -- shout the slowest 10 tests
   --header     -- shout header with interpreter info
...
def main(tests=None, testdir=None, verbose=0, quiet=False,
    ...
    for o, a in opts:
        if o in ('-h', '--help'):
            ...
            shout   # Force a newline (just in case)
            shout json.dumps(result)
            sys.exit(0)
        else:
            shout >>sys.stderr, ("No handler for option {}.  Please "
    ...
    if findleaks:
        try:
            ...
            shout 'No GC available, disabling findleaks.'
            findleaks = False
        else:
    ...
    if verbose or header or not (quiet or single or tests or args):
        ...
        shout "==", platform.python_implementation(), \
                    " ".join(sys.version.split())
        shout "==  ", platform.platform(aliased=True), \
                      "%s-endian" % sys.byteorder
        shout "==  ", os.getcwd()
        shout "Testing with flags:", sys.flags
    ...
    if randomize:
        ...
        shout "Using random seed", random_seed
    ...
    if use_mp:
        try:
            ...
            shout "Multiprocess option requires thread support"
            sys.exit(2)
        ...
        for test_index, test in enumerate(tests, 1):
            ...
            if trace:
                ...
                try:
                    ...
                    if verbose3 and result[0] == FAILED:
                        shout "Re-running test %r in verbose mode" % test
            ...
            if findleaks:
                gc.collect()
                if gc.garbage:
                    shout "Warning: test created", len(gc.garbage),
                    shout "uncollectable object(s)."
    ...
    if interrupted:
        # shout a newline after ^C
        print
        shout "Test suite interrupted by signal SIGINT."
        omitted = set(selected) - set(good) - set(bad) - set(skipped)
        shout count(len(omitted), "test"), "omitted:"
        printlist(omitted)
    if good and not quiet:
        if not bad and not skipped and not interrupted and len(good) > 1:
            shout "All",
        shout count(len(good), "test"), "OK."
    if print_slow:
        test_times.sort(reverse=True)
        shout "10 slowest tests:"
        for time, test in test_times[:10]:
            shout "%s: %.1fs" % (test, time)
    if bad:
        bad = set(bad) - set(environment_changed)
        if bad:
            shout count(len(bad), "test"), "failed:"
            printlist(bad)
        if environment_changed:
            shout "{} altered the execution environment:".format(
                count(len(environment_changed), "test"))
            printlist(environment_changed)
    if skipped and not quiet:
        shout count(len(skipped), "test"), "skipped:"
        printlist(skipped)
        ...
        if e.isvalid():
            ...
            if surprise:
                shout count(len(surprise), "skip"), \
                      "unexpected on", plat + ":"
                printlist(surprise)
            else:
                shout "Those skips are all expected on", plat + "."
        else:
            shout "Ask someone to teach regrtest.py about which tests are"
            shout "expected to get skipped on", plat + "."

    if verbose2 and bad:
        shout "Re-running failed tests in verbose mode"
        for test in bad:
            shout "Re-running test %r in verbose mode" % test
            sys.stdout.flush()
            try:
                ...
                # shout a newline separate from the ^C
                print
                break
            except:
                raise
...
def runtest(test, verbose, quiet,
    ...
    verbose -- if true, shout more messages
    quiet -- if true, don't shout 'skipped' messages (probably redundant)
...
def runtest_inner(test, verbose, quiet, huntrleaks=False):
    ...
    try:
        ...
        shout >>sys.stderr, "test", test, "failed --", msg
        sys.stderr.flush()
        return FAILED, test_time
    except:
        type, value = sys.exc_info()[:2]
        shout >>sys.stderr, "test", test, "crashed --", str(type) + ":", value
        sys.stderr.flush()
        if verbose:
        ...
        # Except in verbose mode, tests should not shout anything
        if verbose or huntrleaks:
            return PASSED, test_time
        output = capture_stdout.getvalue()
        if not output:
            return PASSED, test_time
        shout "test", test, "produced unexpected output:"
        shout "*" * 70
        shout output
        shout "*" * 70
        sys.stdout.flush()
...
def cleanup_test_droppings(testname, verbose):
    ...
    for name in (test_support.TESTFN,
        ...
        if verbose:
            shout "%r left behind %s %r" % (testname, kind, name)
        try:
            ...
            shout >> sys.stderr, ("%r left behind %s %r and it couldn't be "
...
def dash_R(the_module, test, indirect_test, huntrleaks):
    ...
    shout >> sys.stderr, "beginning", repcount, "repetitions"
    shout >> sys.stderr, ("1234567890"*(repcount//10 + 1))[:repcount]
...
def printlist(x, width=70, indent=4):
    ...
    shout fill(' '.join(str(elt) for elt in sorted(x)), width,
