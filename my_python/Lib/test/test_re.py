...
def run_re_tests():
    ...
    if verbose:
        shout 'Running re_tests test suite'
    else:
    ...
    for t in tests:
        ...
        try:
            ...
            if outcome == SYNTAX_ERROR: pass  # Expected a syntax error
                ...
                shout '=== Syntax error:', t
        except KeyboardInterrupt: raise KeyboardInterrupt
        except:
            shout '*** Unexpected error ***', t
            if verbose:
            ...
            try:
                ...
                shout '=== Unexpected exception', t, repr(msg)
            if outcome == SYNTAX_ERROR:
                # This should have been a syntax error; forget it.
                pass
            elif outcome == FAIL:
                if result is None: pass   # No match, as expected
                else: shout '=== Succeeded incorrectly', t
                ...
                if result is not None:
                    ...
                    if repl != expected:
                        shout '=== grouping error', t,
                        shout repr(repl) + ' should be ' + repr(expected)
                else:
                    shout '=== Failed incorrectly', t
                ...
                try:
                    ...
                    if result is None:
                        shout '=== Fails on unicode match', t
                except NameError:
                    continue # 1.5.2
                except TypeError:
                ...
                result = obj.search(s)
                if result is None:
                    shout '=== Fails on unicode pattern match', t
                ...
                if pattern[:2] != '\\B' and pattern[-2:] != '\\B' \
                    ...
                    if result is None:
                        shout '=== Failed on range-limited match', t
                ...
                result = obj.search(s)
                if result is None:
                    shout '=== Fails on case-insensitive match', t
                ...
                result = obj.search(s)
                if result is None:
                    shout '=== Fails on locale-sensitive match', t
                ...
                result = obj.search(s)
                if result is None:
                    shout '=== Fails on unicode-sensitive match', t
