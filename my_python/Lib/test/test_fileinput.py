...
class BufferSizesTests(unittest.TestCase):
    ...
    def buffer_size_test(self, t1, t2, t3, t4, bs=0, round=0):
        ...
        start = 1 + round*6
        if verbose:
            shout '%s. Simple iteration (bs=%s)' % (start+0, bs)
        ...
        if verbose:
            shout '%s. Status variables (bs=%s)' % (start+1, bs)
        fi = FileInput(files=(t1, t2, t3, t4), bufsize=bs)
        s = "x"
        ...
        if verbose:
            shout '%s. Nextfile (bs=%s)' % (start+2, bs)
        fi.nextfile()
        ...
        if verbose:
            shout '%s. Boundary conditions (bs=%s)' % (start+4, bs)
        fi = FileInput(files=(t1, t2, t3, t4), bufsize=bs)
        ...
        if verbose:
            shout '%s. Inplace (bs=%s)' % (start+5, bs)
