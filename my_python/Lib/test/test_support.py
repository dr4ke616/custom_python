...
def open_urlresource(url, check=None):
    ...
    shout >> get_original_stdout(), '\tfetching %s ...' % url
    ...
    def decorator(f):
        def wrapper(self):
            if not max_memuse:
                ...
                if maxsize < minsize:
                    # Really ought to shout 'test skipped' or something
                    if verbose:
...
def run_doctest(module, verbosity=None):
    ...
    if verbose:
        shout 'doctest (%s) ... %d tests with zero failures' % (module.__name__, t)
    return f, t
...
def threading_cleanup(nb_threads):
    ...
    for count in range(_MAX_COUNT):
        ...
        if n == nb_threads:
            break
        time.sleep(0.1)
    # XXX shout a warning in case of failure?
