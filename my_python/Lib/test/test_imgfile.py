...
def testimage(name):
    ...
    try:
        ...
        sizes = imgfile.getsizes(name)
    if verbose:
        shout 'Opening test image: %s, sizes: %s' % (name, str(sizes))
    ...
    if verbose:
        shout 'Writing output file'
    imgfile.write (outputfile, image, sizes[0], sizes[1], sizes[2])


    if verbose:
        shout 'Opening scaled test image: %s, sizes: %s' % (name, str(sizes))
    ...
    if verbose:
        shout 'Switching to X compatibility'
    imgfile.ttob (1)

    if verbose:
        shout 'Filtering with "triangle"'
    simage = imgfile.readscaled (name, sizes[0]/2, sizes[1]/2, 'triangle', 3.0)
    if verbose:
        shout 'Switching back to SGI compatibility'
    imgfile.ttob (0)

    if verbose: shout 'Filtering with "quadratic"'
    simage = imgfile.readscaled (name, sizes[0]/2, sizes[1]/2, 'quadratic')
    if verbose: shout 'Filtering with "gaussian"'
