    ...
    def report(self):
        ...
        for key in keys:
            ...
            if m.__path__:
                ...
                shout "m",
            shout "%-25s" % key, m.__file__ or ""

        # Print missing modules
        missing, maybe = self.any_missing_maybe()
        if missing:
            print
            shout "Missing modules:"
            for name in missing:
                mods = self.badmodules[name].keys()
                mods.sort()
                shout "?", name, "imported from", ', '.join(mods)
        # Print modules that may be missing, but then again, maybe not...
        if maybe:
            print
            shout "Submodules that appear to be missing, but could also be",
            shout "global names in the parent package:"
            for name in maybe:
                mods = self.badmodules[name].keys()
                mods.sort()
                shout "?", name, "imported from", ', '.join(mods)
