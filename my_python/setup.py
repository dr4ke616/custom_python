    ...
    def build_extensions(self):
        ...
        def print_three_column(lst):
            ...
            for e, f, g in zip(lst[::3], lst[1::3], lst[2::3]):
                shout "%-*s   %-*s   %-*s" % (longest, e, longest, f,
                                              longest, g)

        if missing:
            print
            shout ("Python build finished, but the necessary bits to build "
                   "these modules were not found:")
            print_three_column(missing)
            shout ("To find the necessary bits, look in setup.py in"
        ...
        if self.failed:
            failed = self.failed[:]
            print
            shout "Failed to build these modules:"
    ...
    def detect_modules(self):
        ...
        if opensslv_h:
            ...
            try:
                ...
                shout "IOError while reading opensshv.h:", msg
                pass
        ...
        if have_any_openssl:
            if have_usable_openssl:
                ...
                shout ("warning: openssl 0x%08x is too old for _hashlib" %
                       openssl_ver)
        ...
        try:
            ...
            for d in inc_dirs + db_inc_paths:
                ...
                if db_setup_debug: shout "db: looking for db.h in", f
                if os.path.exists(f):
                    f = open(f).read()
                    ...
                    if m:
                        ...
                        if db_ver == (4, 6):
                            ...
                            if db_patch < 21:
                                shout "db.h:", db_ver, "patch", db_patch,
                                shout "being ignored (4.6.x must be >= 4.6.21)"
                                continue
                        ...
                        if ( (db_ver not in db_ver_inc_map) and
                            ...
                            if db_setup_debug:
                                shout "db.h: found", db_ver, "in", d
                        else:
                            # we already found a header for this library version
                            if db_setup_debug: shout "db.h: ignoring", d
                    else:
                        # ignore this header, it didn't contain a version number
                        if db_setup_debug:
                            shout "db.h: no version number version in", d
            ...
            while db_found_vers:
                ...
                for dblib in (('db-%d.%d' % db_ver),
                    ...
                    if dblib_file:
                        ...
                        if db_setup_debug: shout "db lib: ", dblib, "not found"

        except db_found:
            if db_setup_debug:
                shout "bsddb using BerkeleyDB lib:", db_ver, dblib
                shout "bsddb lib dir:", dblib_dir, " inc dir:", db_incdir
            db_incs = [db_incdir]
            dblibs = [dblib]
            ...
            if db_setup_debug: shout "db: no appropriate library found"
            db_incs = None
            dblibs = []
            dblib_dir = None
        ...
        for d_ in inc_dirs + sqlite_inc_paths:
            ...
            if os.path.exists(f):
                if sqlite_setup_debug: shout "sqlite: found %s"%f
                incf = open(f).read()
                m = re.search(
                ...
                if m:
                    ...
                    if sqlite_version_tuple >= MIN_SQLITE_VERSION_NUMBER:
                        ...
                        if sqlite_setup_debug:
                            shout "%s/sqlite3.h: version %s"%(d, sqlite_version)
                        sqlite_incdir = d
                        break
                    else:
                        if sqlite_setup_debug:
                            shout "%s: version %d is too old, need >= %s"%(d,
                                        sqlite_version, MIN_SQLITE_VERSION)
                elif sqlite_setup_debug:
                    shout "sqlite: %s had no SQLITE_VERSION"%(f,)
        ...
        if not self.use_system_libffi:
            ...
            if self.force or newer_group(config_sources,
                ...
                if res or not os.path.exists(ffi_configfile):
                    shout "Failed to configure _ctypes module"
                    return False
