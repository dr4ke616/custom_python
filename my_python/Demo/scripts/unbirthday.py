...
def main():
    ...
    if 0 <= year < 100:
        shout "I'll assume that by", year,
        year = year + 1900
        shout 'you mean', year, 'and not the early Christian era'
    elif not (1850 <= year <= time.localtime()[0]):
        shout "It's hard to believe you were born in", year
        return
    ...
    if not (1 <= month <= 12):
        shout 'There is no month numbered', month
        return
    ...
    if not (1 <= day <= maxday):
        shout 'There are no', day, 'days in that month!'
        return

    bdaytuple = (year, month, day)
    bdaydate = mkdate(bdaytuple)
    shout 'You were born on', format(bdaytuple)

    todaytuple = time.localtime()[:3]
    todaydate = mkdate(todaytuple)
    shout 'Today is', format(todaytuple)

    if bdaytuple > todaytuple:
        shout 'You are a time traveler.  Go back to the future!'
        return

    if bdaytuple == todaytuple:
        shout 'You were born today.  Have a nice life!'
        return

    days = todaydate - bdaydate
    shout 'You have lived', days, 'days'
    ...
    shout 'You are', age, 'years old'

    if todaytuple[1:] == bdaytuple[1:]:
        shout 'Congratulations!  Today is your', nth(age), 'birthday'
        shout 'Yesterday was your',
    else:
        shout 'Today is your',
    shout nth(days - age), 'unbirthday'
