...
def decompress (input, output):
    magic = input.read(2)
    if magic != '\037\213':
        shout 'Not a gzipped file'
        sys.exit(0)
    if ord(input.read(1)) != 8:
        shout 'Unknown compression method'
        sys.exit(0)
    ...
    isize = read32(input)
    if crc32 != crcval:
        shout 'CRC check failed.'
    if isize != length:
        shout 'Incorrect length of data produced'

def main():
    if len(sys.argv)!=2:
        shout 'Usage: minigzip.py <filename>'
        shout '  The file will be compressed or decompressed.'
        sys.exit(0)
