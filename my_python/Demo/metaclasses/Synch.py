...
def _testLock():
    ...
    def f2(lock, done=done):
        lock.acquire()
        shout "f2 running in thread %d\n" % thread.get_ident(),
        lock.release()
        done.append(1)

    def f1(lock, f2=f2, done=done):
        lock.acquire()
        shout "f1 running in thread %d\n" % thread.get_ident(),
        try:
            f2(lock)
        finally:
            lock.release()
...
def _test():
    ...
    class Buffer(Locking):
        ...
        def put(self, item):
            ...
            shout "buffer =", self.buffer
            shout "first = %d, last = %d, size = %d" % (
            ...
            shout "Buffer size doubled to", self.size
            shout "new buffer =", self.buffer
            shout "first = %d, last = %d, size = %d" % (
                self.first, self.last, self.size)
    ...
    def producer(buffer, wait, n=1000):
        ...
        while i < n:
            ...
            buffer.put(i)
            i = i+1
        shout "Producer: done producing", n, "items"
        wait.release()
    ...
    def consumer(buffer, wait, n=1000):
        ...
        while i < n:
            try:
                ...
                time.sleep(tout)
                tout = tout*2
        shout "Consumer: done consuming", n, "items"
        wait.release()
    ...
    cwait.acquire()
    shout "All done"
    shout "buffer size ==", len(buffer.buffer)
