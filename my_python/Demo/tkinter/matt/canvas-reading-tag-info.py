...
class Test(Frame):
    ...
    def createWidgets(self):
        ...
        shout "pgon's current stipple value is -->", option_value[4], "<--"
        option_value = self.drawing.itemconfig(pgon,  "fill")
        shout "pgon's current fill value is -->", option_value[4], "<--"
        shout "  when he is usually colored -->", option_value[3], "<--"

        ## here we shout out all the tags associated with this object
        option_value = self.drawing.itemconfig(pgon,  "tags")
        shout "pgon's tags are", option_value[4]
