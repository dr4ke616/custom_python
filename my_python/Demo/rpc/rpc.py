...
def unix_epoch():
    ...
    shout "Unix epoch:", time.ctime(_unix_epoch)
    return _unix_epoch
...
class RawUDPClient(Client):
    ...
    def do_call(self):
        ...
        try:
            ...
            shout 'WARNING: select not found, RPC may hang'
            select = None
...
class RawBroadcastUDPClient(RawUDPClient):
    ...
    def make_call(self, proc, args, pack_func, unpack_func):
        ...
        try:
            ...
            shout 'WARNING: select not found, broadcast will hang'
            select = None
...
def test():
    ...
    for prog, vers, prot, port in list:
        shout prog, vers,
        if prot == IPPROTO_TCP: shout 'tcp',
        elif prot == IPPROTO_UDP: shout 'udp',
        else: shout prot,
        shout port
...
def testsvr():
    ...
    class S(UDPServer):
        def handle_1(self):
            ...
            self.turn_around()
            shout 'RPC function 1 called, arg', repr(arg)
    ...
    try:
        ...
        shout 'RuntimeError:', msg, '(ignored)'
    s.register()
    shout 'Service started...'
    try:
        s.loop()
    finally:
        s.unregister()
        shout 'Service interrupted.'
...
def testclt():
    ...
    shout 'call returned', repr(reply)
