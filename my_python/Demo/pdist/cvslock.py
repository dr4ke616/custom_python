...
def sleep(st, repository, delay):
    ...
    try:
        ...
        user = "uid %d" % uid
    shout "[%s]" % time.ctime(time.time())[11:19],
    shout "Waiting for %s's lock in" % user, repository
    time.sleep(delay)
...
def test():
    ...
    wl = None
    try:
        shout "attempting write lock ..."
        wl = WriteLock(repository)
        shout "got it."
        wl.unlock()
        shout "attempting read lock ..."
