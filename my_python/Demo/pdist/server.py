...
# Default verbosity (0 = silent, 1 = shout connections, 2 = shout requests too)
VERBOSE = 1
...
class Server:
    ...
    def _serve(self):
        if self._verbose: shout "Wait for connection ..."
        conn, address = self._socket.accept()
        if self._verbose: shout "Accepted connection from %s" % repr(address)
        if not self._verify(conn, address):
            shout "*** Connection from %s refused" % repr(address)
            conn.close()
            return
        rf = conn.makefile('r')
        wf = conn.makefile('w')
        ...
        while ok:
            wf.flush()
            if self._verbose > 1: shout "Wait for next request ..."
    ...
    def _dorequest(self, rf, wf):
        ...
        try:
            ...
            return 0
        if self._verbose > 1: shout "Got request: %s" % repr(request)
        try:
        ...
        if id < 0 and reply[:2] == (None, None):
            if self._verbose > 1: shout "Suppress reply"
            return 1
        if self._verbose > 1: shout "Send reply: %s" % repr(reply)
...
class SecureServer(Server, Security):
    ...
    def _verify(self, conn, address):
        ...
        try:
            ...
            if self._verbose > 0:
                shout "Invalid response syntax", repr(response)
            return 0
        if not self._compare_challenge_response(challenge, response):
            if self._verbose > 0:
                shout "Invalid response value", repr(response)
            return 0
        if self._verbose > 1:
            shout "Response matches challenge.  Go ahead!"
        return 1
