...
class Client:
    ...
    def _pre_init(self, address, verbose = VERBOSE):
        ...
        if self._verbose: shout "Connecting to %s ..." % repr(address)
        self._socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self._socket.connect(address)
        if self._verbose: shout "Connected."
    ...
    def _vsend(self, name, args, wantreply = 1):
        ...
        if self._verbose > 1: shout "sending request: %s" % repr(request)
    ...
    def _vrecv(self, id):
        ...
        if self._replies.has_key(id):
            if self._verbose > 1: shout "retrieving previous reply, id = %d" % id
            reply = self._replies[id]
            del self._replies[id]
            return reply
        aid = abs(id)
        while 1:
            if self._verbose > 1: shout "waiting for reply, id = %d" % id
            rp = pickle.Unpickler(self._rf)
            reply = rp.load()
            del rp
            if self._verbose > 1: shout "got reply: %s" % repr(reply)
            rid = reply[2]
            arid = abs(rid)
            if arid == aid:
                if self._verbose > 1: shout "got it"
                return reply
            self._replies[rid] = reply
            if arid > aid:
                if self._verbose > 1: shout "got higher id, assume all ok"
                return (None, None, id)
