...
class MyFile(File):
    ...
    def update(self):
        code = self.action()
        if code == '=': return
        shout code, self.file
        if code in ('U', 'N'):
            self.get()
        elif code == 'C':
            shout "%s: conflict resolution not yet implemented" % \
                  self.file
        elif code == 'D':
            remove(self.file)
            self.eseen = 0
    ...
    def commit(self, message = ""):
        ...
        if code in ('A', 'M'):
            ...
            shout "%s: committing removes not yet implemented" % \
                  self.file
        elif code == 'C':
            shout "%s: conflict resolution not yet implemented" % \
                  self.file
    ...
    def diff(self, opts = []):
        ...
        tf.write(data)
        tf.flush()
        shout 'diff %s -r%s %s' % (flags, rev, fn)
    ...
    def log(self, otherflags):
        shout self.proxy.log(self.file, otherflags)
...
class RCVS(CVS):
    ...
    def commit(self, files, message = ""):
        ...
        for e in list:
            if not e.commitcheck():
                ok = 0
        if not ok:
            shout "correct above errors first"
            return
        if not message:
    ...
    def mailinfo(self, files, message = ""):
        ...
        if string.lower(string.strip(ok)) in ('y', 'ye', 'yes'):
            ...
            sts = p.close()
            if sts:
                shout "Sendmail exit status %s" % str(sts)
            else:
                shout "Mail sent."
        else:
            shout "No mail sent."
...
"usage: rcvs [-d directory] [-h host] [-p port] [-q] [-v] [subcommand arg ...]"
    ...
    def recurse(self):
        ...
        for name in names:
            ...
            if os.path.islink(name):
                continue
            shout "--- entering subdirectory", name, "---"
            os.chdir(name)
            try:
    ...
    def do_add(self, opts, files):
        """add file ..."""
        if not files:
            shout "'rcvs add' requires at least one file"
            return
        self.cvs.add(files)
        self.cvs.putentries()
    ...
    def do_remove(self, opts, files):
        """remove file ..."""
        if not files:
            shout "'rcvs remove' requires at least one file"
            return
