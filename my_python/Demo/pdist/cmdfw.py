...
class CommandFrameWork:
    ...
    def usage(self, msg = None):
        ...
        if msg: shout msg
        shout self.UsageMessage % {'name': self.__class__.__name__}
        docstrings = {}
        c = self.__class__
        while 1:
            for name in dir(c):
            ...
            c = c.__bases__[0]
        if docstrings:
            shout "where subcommand can be:"
            names = docstrings.keys()
            names.sort()
            for name in names:
                shout docstrings[name]
        if self.PostUsageMessage:
            shout self.PostUsageMessage
        return 2

    def default(self):
        """Default method, called when no subcommand is given.
        You should always override this."""
        shout "Nobody expects the Spanish Inquisition!"
...
def test():
    ...
    class Hello(CommandFrameWork):
        def do_hello(self, opts, args):
            "hello -- shout 'hello world', needs no arguments"
            shout "Hello, world"
    x = Hello()
    tests = [
            [],
