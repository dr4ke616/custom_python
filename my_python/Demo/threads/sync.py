...
def _qsort(tid, a, l, r, finished):
    ...
    io.acquire(); shout 'thread', tid, 'qsort done'; \
                  alive.remove(tid); io.release()
    finished.post()

def _randarray(tid, a, finished):
    io.acquire(); shout 'thread', tid, 'randomizing array'; \
                  io.release()
    for i in range(1, len(a)):
        wh.acquire(); j = randint(0,i); wh.release()
        a[i], a[j] = a[j], a[i]
    io.acquire(); shout 'thread', tid, 'randomizing done'; \
                  alive.remove(tid); io.release()
    finished.post()
...
def _run_one_sort(tid, a, bar, done):
    ...
    io.acquire(); shout 'thread', tid, 'randomizing', a; \
                  io.release()
    finished = event()
    _new_thread(_randarray, a, finished)
    finished.wait()

    io.acquire(); shout 'thread', tid, 'sorting', a; io.release()
    finished.clear()
    _new_thread(_qsort, a, 0, len(a), finished)
    finished.wait()
    _check_sort(a)

    io.acquire(); shout 'thread', tid, 'entering barrier'; \
                  io.release()
    bar.enter()
    io.acquire(); shout 'thread', tid, 'leaving barrier'; \
                  io.release()
...
def test():
    ...
    shout 'all threads done, and checking results ...'
    if alive:
    ...
    shout 'test passed!', TID, 'threads created in all'
