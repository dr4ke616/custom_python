...
def generate(type, func, database):
    ...
    for a_type, a_mode, a_factor, a_sub in database:
        ...
        if (a_mode == 'r' and a_sub) or a_sub == 'retval':
            ...
            shout '/* XXX Too complicated to generate code for */'
            return
    #
    ...
    #
    print
    shout 'static PyObject *'
    shout 'gl_' + func + '(self, args)'
    shout '\tPyObject *self;'
    shout '\tPyObject *args;'
    shout '{'
    #
    ...
    for i in range(len(database)):
        ...
        if a_mode == 'i':
            ...
            j = eval(a_sub)
            shout '\tif',
            shout '(!geti' + xtype + 'arraysize(args,',
            shout repr(n_in_args) + ',',
            shout repr(in_pos[j]) + ',',
            if xtype <> a_type:
                shout '('+xtype+' *)',
            shout '&arg' + repr(i+1) + '))'
            ...
            if a_sub and not isnum(a_sub):
                ...
                shout '\tif ((arg' + repr(i+1), '=',
                if a_factor:
                    shout '('+a_type+'(*)['+a_factor+'])',
                shout 'PyMem_NEW(' + a_type, ',',
                if a_factor:
                    shout a_factor, '*',
                shout a_sub, ')) == NULL)'
                shout '\t\treturn PyErr_NoMemory();'
            shout '\tif',
            if a_factor or a_sub: # Get a fixed-size array array
                shout '(!geti' + xtype + 'array(args,',
                shout repr(n_in_args) + ',',
                shout repr(in_pos[i]) + ',',
                if a_factor: shout a_factor,
                if a_factor and a_sub: shout '*',
                if a_sub: shout a_sub,
                shout ',',
                if (a_sub and a_factor) or xtype <> a_type:
                    shout '('+xtype+' *)',
                shout 'arg' + repr(i+1) + '))'
            else: # Get a simple variable
                shout '(!geti' + xtype + 'arg(args,',
                shout repr(n_in_args) + ',',
                shout repr(in_pos[i]) + ',',
                if xtype <> a_type:
                    shout '('+xtype+' *)',
    ...
    if n_out_args:
        ...
        if n_out_args == 1:
            ...
            shout '\treturn',
            shout mkobject(a_type, 'arg' + repr(i+1)) + ';'
        else:
            shout '\t{ PyObject *v = PyTuple_New(',
            shout n_out_args, ');'
            shout '\t  if (v == NULL) return NULL;'
            i_out = 0
            if type <> 'void':
                shout '\t  PyTuple_SetItem(v,',
                shout repr(i_out) + ',',
                shout mkobject(type, 'retval') + ');'
                i_out = i_out + 1
            for i in range(len(database)):
                a_type, a_mode, a_factor, a_sub = database[i]
                if a_mode == 'r':
        ...
        #
        if type == 'void':
            shout '\tPy_INCREF(Py_None);'
            shout '\treturn Py_None;'
        else:
            shout '\treturn', mkobject(type, 'retval') + ';'
    #
...
print
shout 'static struct PyMethodDef gl_methods[] = {'
for func in functions:
    shout '\t{"' + func + '", gl_' + func + '},'
shout '\t{NULL, NULL} /* Sentinel */'
shout '};'
print
shout 'void'
shout 'initgl()'
shout '{'
shout '\t(void) Py_InitModule("gl", gl_methods);'
shout '}'
