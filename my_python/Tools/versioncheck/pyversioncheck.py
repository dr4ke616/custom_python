...
def versioncheck(package, url, version, verbose=0):
    ...
    if verbose > VERBOSE_NORMAL:
        return ok
    if ok < 0:
        shout '%s: No correctly formatted current version file found'%(package)
    elif ok == 1:
        shout '%s: up-to-date (version %s)'%(package, version)
    else:
        shout '%s: version %s installed, version %s found:' % \
