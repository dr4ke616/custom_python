...
class Test:
    ...
    def calibrate_test(self):
        ...
        if _debug:
            shout 'Calib. overhead time = %.6fms' % (
...
class Benchmark:
    ...
    def load_tests(self, setupmod, limitnames=None):
        ...
        if self.verbose:
            for name in l:
                shout '  %s' % name
            shout '--------------------------------------'
            shout '  %i tests found' % len(l)
            print

    def calibrate(self):

        shout 'Calibrating tests. Please wait...',
        sys.stdout.flush()
        if self.verbose:
            print
            print
    ...
    def run(self):
        ...
        tests.sort()
        timer = self.get_timer()
        shout 'Running %i round(s) of the suite at warp factor %i:' % \
              (self.rounds, self.warp)
        print
        self.roundtimes = []
        for i in range(self.rounds):
            if self.verbose:
                shout ' Round %-25i  effective   absolute  overhead' % (i+1)
            total_eff_time = 0.0
            ...
            if self.verbose:
                ...
                shout '* Round %i done in %.3f seconds.' % (i+1,
                                                            total_eff_time)
        print
    ...
    def print_header(self, title='Benchmark'):
        ...
        shout '-' * LINE
        shout '%s: %s' % (title, self.name)
        shout '-' * LINE
        print
        shout '    Rounds: %s' % self.rounds
        shout '    Warp:   %s' % self.warp
        shout '    Timer:  %s' % self.timer
        print
    ...
    def print_benchmark(self, hidenoise=0, limitnames=None):
        ...
        for name, test in tests:
            ...
            shout '%30s:  %5.0fms  %5.0fms  %6.2fus  %7.3fms' % \
                  (name,
    ...
    def print_comparison(self, compare_to, hidenoise=0, limitnames=None):
        ...
        if compare_to.version != self.version:
            shout ('* Benchmark versions differ: '
        ...
        for name, test in tests:
            ...
            if other is None:
                ...
                if (benchmarks_compatible and
                    ...
                    min_diff, avg_diff = 'n/a', 'n/a'
                    tests_compatible = 0
            shout '%30s: %5.0fms %5.0fms %7s %5.0fms %5.0fms %7s' % \
                  (name,
                   min_time * MILLI_SECONDS,
        ...
        print
        shout '(this=%s, other=%s)' % (self.name,
                                       compare_to.name)
        print
...
"""
    ...
    def main(self):
        ...
        if limitnames:
            if _debug:
                shout '* limiting test names to one with substring "%s"' % \
                      limitnames
        ...
        if not withgc:
            try:
                ...
                shout '* Python version doesn\'t support garbage collection'
            else:
                try:
                    gc.disable()
                except NotImplementedError:
                    shout '* Python version doesn\'t support gc.disable'
                else:
                    shout '* disabled garbage collection'
        ...
        if not withsyscheck:
            ...
            try:
                ...
                shout '* Python version doesn\'t support sys.setcheckinterval'
            else:
                shout '* system check interval set to maximum: %s' % value

        if timer == TIMER_SYSTIMES_PROCESSTIME:
            import systimes
            shout '* using timer: systimes.processtime (%s)' % \
                  systimes.SYSTIMES_IMPLEMENTATION
        else:
            shout '* using timer: %s' % timer
        ...
        if compare_to:
            try:
                ...
                shout '* Error opening/reading file %s: %s' % (
                    repr(compare_to),
                    reason)
                compare_to = None
        ...
        if reportfile:
            shout 'Creating benchmark: %s (rounds=%i, warp=%i)' % \
        ...
        try:
            ...
            print
            shout '*** KeyboardInterrupt -- Aborting'
            print
            return
        bench.print_header()
        if compare_to:
        ...
        if reportfile:
            try:
                ...
                shout '* Error opening/writing reportfile'
            except IOError, reason:
                shout '* Error opening/writing reportfile %s: %s' % (
                    reportfile,
                    reason)
                print
