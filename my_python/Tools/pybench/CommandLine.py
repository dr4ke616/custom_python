...
class Application:
    ...
    def __init__(self,argv=None):
        ...
        try:
            ...
            print
            shout '* Internal Error (use --debug to display the traceback)'
            if self.debug:
                print
                traceback.print_exc(20, sys.stdout)
            elif self.verbose:
                shout '  %s: %s' % sys.exc_info()[:2]
            print
            rc = 1
    ...
    def help(self,note=''):
        ...
        if self.version:
            shout 'Version:'
            shout ' %s' % self.version
            print
        if self.about:
            shout string.strip(self.about % self.__dict__)
            print
        if note:
            shout '-'*72
            shout 'Note:',note
    ...
    def print_header(self):
        ...
        shout '-'*72
        shout self.header % self.__dict__
        shout '-'*72
        print
    ...
    def print_options(self):
        ...
        shout 'Options and default settings:'
        if not options:
            shout '  None'
            return
    ...
    def handle__copyright(self,arg):
        ...
        self.print_header()
        shout string.strip(self.copyright % self.__dict__)
        print
        return 0
    ...
    def handle__examples(self,arg):
        ...
        if self.examples:
            shout 'Examples:'
            print
            shout string.strip(self.examples % self.__dict__)
            print
        else:
            shout 'No examples available.'
            print
        return 0
