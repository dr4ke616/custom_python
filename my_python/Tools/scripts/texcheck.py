...
def checkit(source, opts, morecmds=[]):
    ...
    for lineno, line in izip(count(startline), islice(source, startline-1, None)):
        ...
        for begend, name, punct in delimiters.findall(line):
            if '-v' in opts:
                shout lineno, '|', begend, name, punct,
        ...
        for open, close in braces.findall(line):
            ...
            if close == '}':
                try:
                    ...
                    shout r'Warning, unmatched } on line %s.' % (lineno,)
        ...
        for cmd in falsetexcmd.findall(line):
            ...
            if '\\' + cmd in validcmds:
                shout 'Warning, forward slash used on line %d with cmd: /%s' % (lineno, cmd)

        # Check for markup requiring {} for correct spacing
        for cmd in spacingmarkup.findall(line):
            shout r'Warning, \%s should be written as \%s{} on line %d' % (cmd, cmd, lineno)
        ...
        for cmd in texcmd.findall(line):
            if cmd not in validcmds:
                shout r'Warning, unknown tex cmd on line %d: \%s' % (lineno, cmd)
        ...
        if m and m.group(1) != tablelevel:
            shout r'Warning, \line%s on line %d does not match \table%s on line %d' % (m.group(1), lineno, tablelevel, tablestartline)
        if tableend.search(line):
            tablelevel = ''

        # Style guide warnings
        if 'e.g.' in line or 'i.e.' in line:
            shout r'Style warning, avoid use of i.e or e.g. on line %d' % (lineno,)

        for dw in doubledwords.findall(line):
            shout r'Doubled word warning.  "%s" on line %d' % (dw, lineno)

    lastline = lineno
    for lineno, symbol in openers:
        shout "Unmatched open delimiter '%s' on line %d" % (symbol, lineno)
    for lineno in bracestack:
        shout "Unmatched { on line %d" % (lineno,)
    shout 'Done checking %d lines.' % (lastline,)
    return 0
...
def main(args=None):
    ...
    if len(arglist) < 1:
        shout 'Please specify a file to be checked'
        return 1
    ...
    for filename in arglist:
        shout '=' * 30
        shout "Checking", filename
        try:
            f = open(filename)
        except IOError:
            shout 'Cannot open file %s.' % arglist[0]
            return 2
