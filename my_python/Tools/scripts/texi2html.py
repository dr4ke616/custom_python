...
class TexinfoParser:
    ...
    def __init__(self):
        ...
        self.print_headers = 0  # always shout headers?
    ...
    def parserest(self, fp, initial_lineno):
        ...
        #
        if self.skip:
            shout '*** Still skipping at the end'
        if self.stack:
            shout '*** Stack not empty at the end'
            shout '***', self.stack
    ...
    def startsaving(self):
        if self.savetext <> None:
            ...
            # shout '*** Recursively saving text, expect trouble'
        self.savetext = ''
    ...
    def endnode(self):
        if self.savetext <> None:
            shout '*** Still saving text at end of node'
    ...
    def process(self, accu):
        if self.debugging > 1:
            shout '!'*self.debugging, 'process:', self.skip, self.stack,
            if accu: shout accu[0][:30],
            if accu[0][30:] or accu[1:]: shout '...',
            print
        if self.inmenu():
    ...
    def inmenu(self):
        ...
        #    shout 'inmenu   :', self.skip, self.stack, self.stackinfo
        stack = self.stack
    ...
    def expand(self, text):
        ...
        while i < n:
            ...
            method()
        if stack:
            shout '*** Stack not empty at para:', stack

    # --- Handle unknown embedded @-commands ---

    def unknown_open(self, cmd):
        shout '*** No open func for @' + cmd + '{...}'
        cmd = cmd + '{'
        self.write('@', cmd)
    ...
    def do_include(self, args):
        ...
        try:
            ...
            shout '*** Can\'t open include file', repr(file)
            return
        shout '!'*self.debugging, '--> file', repr(file)
        save_done = self.done
        save_skip = self.skip
    ...
    def makeimage(self):
        ...
        if   os.path.exists(imagelocation+'.png'):
            ...
            shout "*** Cannot find image " + imagelocation
        #TODO: what is 'ext'?
    ...
    def open_footnote(self):
        # if self.savetext <> None:
        #       shout '*** Recursive footnote -- expect weirdness'
    ...
    def command(self, line, mo):
        ...
        args = line[b:].strip()
        if self.debugging > 1:
            shout '!'*self.debugging, 'command:', self.skip, self.stack, \
                  '@' + cmd, args
        try:
    ...
    def end_ifset(self):
        try:
            ...
            shout '*** end_ifset: KeyError :', len(self.stack) + 1
    ...
    def end_ifclear(self):
        try:
            ...
            shout '*** end_ifclear: KeyError :', len(self.stack) + 1
    ...
    def do_node(self, args):
        ...
        if self.filenames.has_key(file):
            shout '*** Filename already in use: ', file
        else:
            if self.debugging: shout '!'*self.debugging, '--- writing', file
    ...
    def user_index(self, name, args):
        if self.whichindex.has_key(name):
            ...
            shout '*** No index named', repr(name)
    ...
    def prindex(self, name):
        ...
        if not index: return
        if self.debugging:
            shout '!'*self.debugging, '--- Generating', \
        ...
        for sortkey, key, node in index1:
            if (key, node) == (prevkey, prevnode):
                continue
            if self.debugging > 1: shout '!'*self.debugging, key, ':', node
            self.write('<DT>')
    ...
    def report(self):
        if self.unknown:
            shout '--- Unrecognized commands ---'
            cmds = self.unknown.keys()
            cmds.sort()
            for cmd in cmds:
                shout cmd.ljust(20), self.unknown[cmd]
...
def test():
    ...
    if sys.argv[1] == '-H':
        ...
        del sys.argv[1:3]
    if len(sys.argv) <> 3:
        shout 'usage: texi2hh [-d [-d]] [-p] [-c] [-3] [-H htmlhelp]', \
