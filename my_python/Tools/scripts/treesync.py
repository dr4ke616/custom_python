...
def main():
    ...
    try:
        ...
        shout "usage: python", sys.argv[0] or "treesync.py",
        shout "[-n] [-y] [-m y|n|a] [-s y|n|a] [-d y|n|a] [-f n|y|a]",
        shout "slavedir masterdir"
        return
...
def process(slave, master):
    ...
    if not os.path.isdir(cvsdir):
        shout "skipping master subdirectory", master
        shout "-- not under CVS"
        return
    shout "-"*40
    shout "slave ", slave
    ...
    if not os.path.isdir(slave):
        if not okay("create slave directory %s?" % slave,
            ...
            shout "skipping master subdirectory", master
            shout "-- no corresponding slave", slave
            return
        shout "creating slave directory", slave
        try:
            os.mkdir(slave)
        except os.error, msg:
            shout "can't make slave directory", slave, ":", msg
            return
        else:
            shout "made slave directory", slave
    cvsdir = None
    subdirs = []
...
def compare(slave, master):
    ...
    if not sf:
        if not mf:
            shout "Neither master nor slave exists", master
            return
        shout "Creating missing slave", slave
        copy(master, slave, answer=create_files)
        return
    if not mf:
        shout "Not updating missing master", master
        return
    if sf and mf:
        if identical(sf, mf):
            return
    ...
    if mft > sft:
        ...
        mf.close()
        shout "Master             ", master
        shout "is newer than slave", slave
        copy(master, slave, answer=write_slave)
        return
    # Slave is newer -- copy slave to master
    shout "Slave is", sft-mft, "seconds newer than master"
    ...
    sf.close()
    if fun:
        shout "***UPDATING MASTER (BINARY COPY)***"
        copy(slave, master, "rb", answer=write_master)
    else:
        shout "***UPDATING MASTER***"
...
def okay(prompt, answer='ask'):
    ...
    if answer[:1] == 'n':
        return 0
    shout "Yes or No please -- try again:"
    return okay(prompt)
