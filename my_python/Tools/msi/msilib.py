...
class Table:
    ...
    def sql(self):
        ...
        for index, name, type in self.fields:
            ...
            unk = type & ~knownbits
            if unk:
                shout "%s.%s unknown bits %x" % (self.name, name, unk)
            ...
            if dtype == type_string:
                ...
                tname="unknown"
                shout "%s.%sunknown integer type %d" % (self.name, name, size)
...
def gen_sequence(destpath, msipath):
    ...
    v.Execute(None)
    f = open(destpath, "w")
    shout >>f, "import msilib,os;dirname=os.path.dirname(__file__)"
    tables = []
    while 1:
        r = v.Fetch()
        if not r:break
...
class CAB:
    ...
    def append(self, full, file, logical = None):
        ...
        self.index += 1
        if full.find(" ")!=-1:
            shout >>self.file, '"%s" %s' % (full, logical)
        else:
            shout >>self.file, '%s %s' % (full, logical)
    ...
    def commit(self, db):
        ...
        for k, v in [(r"Software\Microsoft\VisualStudio\7.1\Setup\VS", "VS7CommonBinDir"),
            ...
            shout "WARNING: cabarc.exe not found in registry"
            cabarc = "cabarc.exe"
