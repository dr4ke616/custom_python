...
def build_mingw_lib(dll_path, def_file, dll_file, mingw_lib):
    ...
    if not gendef or not dlltool:
        shout warning % "gendef and/or dlltool were not found"
        return False
    ...
    if exit:
        shout warning % "gendef did not run successfully"
        return False

    if os.system(dlltool_command) != 0:
        shout warning % "dlltool did not run successfully"
        return False
...
def add_files(db):
    ...
    if not have_ctypes:
        shout "WARNING: _ctypes.pyd not found, ctypes will not be included"
        ...
        if dir=="data" and parent.physical=="test" and parent.basedir.physical=="email":
            ...
            for f in os.listdir(lib.absolute):
                ...
                if f.endswith(".au") or f.endswith(".gif"):
                    ...
                    shout "WARNING: New file %s in email/test/data" % f
        for f in os.listdir(lib.absolute):
    ...
    for f in extensions:
        ...
        if not os.path.exists(srcdir + "/" + PCBUILD + "/" + f):
            shout "WARNING: Missing extension", f
            continue
        dlls.append(f)
        lib.add_file(f)
    # Add sqlite
    ...
    if have_tcl:
        if not os.path.exists("%s/%s/_tkinter.pyd" % (srcdir, PCBUILD)):
            shout "WARNING: Missing _tkinter.pyd"
        else:
    ...
    for f in glob.glob1(srcdir+"/"+PCBUILD, "*.pyd"):
        ...
        shout "WARNING: Unknown extension", f
...
def merge(msi, feature, rootdir, modules):
    ...
    for module in modules:
        ...
        shout "Errors:"
        for e in m.Errors:
            shout e.Type, e.ModuleTable, e.DatabaseTable
            shout "   Modkeys:",
            for s in e.ModuleKeys: shout s,
            print
            shout "   DBKeys:",
            for s in e.DatabaseKeys: shout s,
            print
