...
def send_my_cookie(ui):
    ...
    shout "Set-Cookie: %s=%s; path=%s;" % (name, value, path),
    shout time.strftime("expires=%a, %d-%b-%y %X GMT", gmt)
...
class FaqWizard:
    ...
    def last_changed(self, files):
        ...
        for file in files:
            ...
            if entry:
                ...
                if mtime > latest:
                    latest = mtime
        shout time.strftime(LAST_CHANGED, time.localtime(latest))
        emit(EXPLAIN_MARKS)
    ...
    def cantcommit(self):
        ...
        shout CANTCOMMIT_HEAD
        self.errordetail()
        shout CANTCOMMIT_TAIL
