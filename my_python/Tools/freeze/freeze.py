...
def main():
    ...
    if ishome:
        shout "(Using Python source directory)"
        binlib = exec_prefix
    ...
    if odir and not os.path.isdir(odir):
        try:
            os.mkdir(odir)
            shout "Created output directory", odir
        except os.error, msg:
    ...
    if odir:
        shout 'Now run "make" in', odir,
        shout 'to build the target:', base_target
    else:
        shout 'Now run "make" to build the target:', base_target
...
def usage(msg):
    ...
    shout "Error:", msg
    shout "Use ``%s -h'' for help" % sys.argv[0]
    sys.exit(2)
