...
def hexrepr(t, precision=4):
    ...
    try:
        ...
        shout '* failed to convert %r: %s' % (t, why)
        raise
...
def rewritepythondir(dir, dirprefix='', comments=1):
    ...
    for mapname in mapnames:
        ...
        codefile = name + '.py'
        shout 'converting %s to %s' % (mapname,
                                       dirprefix + codefile)
        try:
